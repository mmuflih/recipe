        ...
        <div class="card-footer">
          <div class="btn-toolbar" role="toolbar" aria-label="Toolbar with button groups">
            <div class="button-group mr-2">
              <a href="#/categories" class="btn btn-info btn-icon-split">
...
export default {
  ...
  created () {
    ...
    if (query && query.slug) {
      HTTP.get(`/v1/categories/slug/${query.slug}`).then(res => {
  ...
  methods: {
    save () {
      this.loading = true
      HTTP.post('/v1/categories', this.data).then(res => {
        this.loading = false
        this.$toast.success('Berhasil menambahkan kategori').goAway()
    ...
    edit () {
      this.loading = true
      HTTP.put('/v1/categories/' + this.data.id, this.data).then(res => {
        this.loading = false
